...
void CArgosRosBot::Init(TConfigurationNode& t_node) {
  ...
  stringstream puckListTopic, proximityTopic, distScanTopic;
  ...
  stringstream cmdVelTopic, ledsTopic, distScanAcrTopic;//, gripperTopic;
  cmdVelTopic << "/" << GetId() << "/cmd_vel";
//  gripperTopic << "/" << GetId() << "/gripper";
  ledsTopic << "/" << GetId() << "/leds";
  distScanAcrTopic << "/" << GetId() << "/distScanControler";
...
//  gripperSub = nodeHandle->subscribe(gripperTopic.str(), 1, &CArgosRosBot::gripperCallback, this);
  ledsSub = nodeHandle->subscribe(ledsTopic.str(), 1, &CArgosRosBot::ledsCallback, this);
...
*/
void CArgosRosBot::ledsCallback(const LedsColor color)
{
